...
def parse_multiple_pdfs(pdf_paths):
    """Parse multiple PDFs and combine all questions."""
    all_questions = []
    seen_ids = set()
    ...
    for pdf_path in pdf_paths:
        ...
        print(f"  -> Found {len(questions)} questions ({new_count} new, {duplicate_count} duplicates)")

    return all_questions


if __name__ == "__main__":
    # Option 1: Specify PDF files explicitly
    pdf_files = [
        "200conventions.pdf",
        "200craft&structure.pdf",
        "200expression_of_ideas.pdf",
        "200information&ideas.pdf",
    ]

    output_json = "parsed_sat_questions.json"

    print(f"Found {len(pdf_files)} PDF files to process\n")
    results = parse_multiple_pdfs(pdf_files)

    with open(output_json, "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n✓ Extracted {len(results)} total unique questions → {output_json}")